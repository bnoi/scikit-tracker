...
def test_radial_speed():
    ...
    rd_speeds = radial_speed(trajs, in_coords=['x', 'y'], from_polar=False)
...
def test_polar_coords():
    ...
    polar = get_polar_coords(trajs, get_dtheta=True)
...
def test_continuous_theta_1d():
    ...
    thetas_out, dthetas = continuous_theta(thetas_in,
                                           return_dtheta=True)
...
def test_continuous_theta_2d():
    ...
    thetas_out, dthetas = continuous_theta(thetas_in,return_dtheta=True)
    thetas_out2, dthetas2 = continuous_theta(thetas_in.T, return_dtheta=True, axis=0)
